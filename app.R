...
data$rating <- ifelse(data$quality > 6, "excellent",
                      ifelse(data$quality <= 6 & data$quality >= 5, "good", "poor"))
data$quality <- NULL
...
# user interface
ui <- {dashboardPage(
  dashboardHeader(title = "Wine Quality Dashboard", titleWidth = 450),
  dashboardSidebar(
    sidebarMenu(
  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "intro",
              fluidPage(fluidRow(box(img(src='winecountry.jpg', align = "left"), title = "Predicting Wine Preferences Based on Physiochemical Properties", width = 6),
              ...
              fluidRow(box('K-Nearest Neighbors is a useful non-parametric algorithm for classification and regression problems. I implemented KNN utilizing the caret library and cross validation training control. Cross validation is helpful when estimating model accuracy and ability to generalize to new data. I chose a 10 fold cross validation method for simplicity and the algorithm optimized KNN on 7 nearest neighbors. K-Nearest Neighbors is the first model I ran and it resulted in an misclassification rate of 20.41% on the testing data.', title = "K-Nearest Neighbors"),
      ...
      tabItem(tabName = "dist",
              ...
              fluidRow(box(sliderInput("n", label = h3("Number of Observations:"), 
                                       min = 50, max = nrow(white), value = 50, ticks = FALSE), width = 6),
                       box(h3("The histograms below show a similar distribution for red and white wine ratings:"))
              ),
...
server <- function(input, output) {
  ...
  output$redPlot <- renderPlotly({
    ...
    ggplotly(distred, tooltip = c("x", "y"))
  })
  ...
  output$corplot1 <- renderPlot({
    ...
    # There is correlation between predictors, therefore I will use feature selection methods to 
    # improve the model. 
  })
