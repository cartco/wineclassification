...
{ 
  ...
  # train the model
    set.seed(12345)
    knn1 <- train(as.factor(rating) ~., data = train, trControl = train_control, method = "knn")
  # classification
    predknn <- predict(knn1, test)
    knnconf <- confusionMatrix(predknn, test.rating)
    df2 <- data.frame(knn1$results[,1:2])
  # decision Tree
    set.seed(12345)
    d.tree <- rpart(rating ~., data = train, method = "class")
    dpred <- predict(d.tree, test, type = "class")
  # reading in the random forest as .rds file to reduce dashboard render time
  # random forest 
    rf1 <- readRDS("rf1wine.rds")
  ...
  dashboardPage(
    ...
    dashboardSidebar(
         sidebarMenu(
                  ...
                  menuItem("K-Nearest Neighbors", tabName = "knn", icon = icon("glyphicon glyphicon-asterisk", lib = "glyphicon")),
                  menuItem("Decision Tree", tabName = "str", icon = icon("glyphicon glyphicon-tree-deciduous", lib = "glyphicon")),
                  menuItem("Random Forest", tabName = "rfimp", icon = icon("glyphicon glyphicon-tree-conifer", lib = "glyphicon"))
                    )
                  ),
    # dashboard body
    dashboardBody(
         tabItems(
            ...
            tabItem(tabName = "str",
                  h2("Decision Tree"),
                  fluidPage(
                    fluidRow(box(plotOutput("decisiontree", width = "100%"), width = 6),
                             box(plotOutput("dsize", width = "100%"), width = 6)),
                    fluidRow(box(verbatimTextOutput("dtout")),
                             ...
                             box(verbatimTextOutput("dconf"), title = "Confusion Matrix")
                    )
                  ),
            # random forest 
            tabItem(tabName = "rfimp",
                  h2("Random Forest"),
                    fluidRow(box(dataTableOutput("dt1"), title = "Random Forest Cross Validation Results"),
                             box(plotOutput("rfvarimportance", width = "100%"))),
                    fluidRow(box(verbatimTextOutput("confusion"), title = "Confusion Matrix"),
                             box('While the KNN model and decision tree performed well, I decided to continue testing other methods of classification. Random forests build on many decision trees by taking advantage of the overfitting problem. With 3000 trees and 6 variables sampled as candidates at each split, I improved the misclassifcation rate to approximately 15.5%. A 5% improvement in classification problems is significant and the random forest proved to be the best model. In addition to improving accuracy, the random forest package in R includes a variable importance plotting function which is useful when improving feature selection. As shown in the graph, Alcohol, Volatile Acidity, Density, Free Sulfur Dioxide, Sulphates, and Residual Sugar were significant in the model. As previously discussed, the type of wine did not influence the quality score and rating. The confusion matrix function in the caret package provides more insight into model effectiveness on a per class basis. There were only a few cases of poor wines in the testing set and the detection rate for this model was suprisingly low. This result confirms my belief that further testing must focus on outlier detection in order to classify the poorly rated wines. The excellent rated wines were classified at a higher rate than the poor wines, but this detection rate could still be improved.', title = "Random Forest Results"))
                  )
              )
          ), skin = "red"
     )
...
server <- function(input, output) {
  ...
  # knn output
  output$knnout <- renderPrint({
    print(knn1)
  })
  # knn confusion matrix
  output$knnconf <- renderPrint({
    print(knnconf)
  })
  # knn performance
  output$knnper <- renderPlot({
    x <- plot(df2, type = "b", main = "KNN Performance", xlab = "K Neighbors")
    print(x)
  })
  # plot decision tree
  ...
  output$dtout <- renderPrint({
    printcp(d.tree)
  })
  output$dsize <- renderPlot({
    plotcp(d.tree)
  })
  output$dconf <- renderPrint({
    confusionMatrix(dpred, test.rating)
  })
  # random forest cross validation results
  output$dt1 <- renderDataTable({
    rfcvresults <- readRDS("rfcvresults.rds")
    rfcverror <- data.frame(rfcvresults$error.cv)
    dt1 <- setDT(rfcverror, keep.rownames = TRUE)[]
    colnames(dt1) <- c("Number of Predictors", "error.cv")
    dt1
    }) 
  ...
  output$confusion <- renderPrint({
    confusionMatrix(data = rfpred, test.rating)
    })
}
